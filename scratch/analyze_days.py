...
def analyze_history():
    print("="*90)
    print(" [HISTORICAL DIAGNOSTIC] ANALYZING RECENT 1000 BARS (3.5 DAYS) ")
    print("="*90)
    ...
    btc_ohlcv = exchange.fetch_ohlcv("BTC/USDT", "1h", limit=300)
    if not btc_ohlcv:
        print("[ERROR] Failed to fetch BTC history.")
        return
    btc_df = pd.DataFrame(btc_ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    
    # Pre-calculate BTC EMA 200 curve on the 1h chart
    ...
    print(f"Analyzing {len(symbols)} symbols over the last 1000 candles (5m timeframe)...")
    print("-" * 120)
    ...
    for symbol in symbols:
        try:
            ...
            for i in range(50, total_candles):
                ...
                # Check if oversold
                wt_oversold = c_wt2 <= strategy.wt_oversold
                if not wt_oversold:
                    continue
                ...
                # BTC bias at timestamp (mapped to active 1h candle start)
                ts = df['timestamp'].iloc[i]
                hour_ts = ts - (ts % 3600000)
                btc_bullish = btc_history_map.get(hour_ts, True)
            ...
            print(f"{symbol:<10} | Min WT2: {min_wt2_value:<6.1f} | WT X-overs: {crossover_count:<3} | OS X-overs: {oversold_crossover_count:<3} | BUY: {buy_signals:<3}")
